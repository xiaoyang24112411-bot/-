...
旧版头像表情指令继续支持不带 `/` 或带 `/`，例如 `摸摸`、`/亲亲 @群友`。发送 `/表情列表` 查看旧版完整清单。新版官方 `nonebot-plugin-memes` 使用独立的 `#` 前缀，避免同一消息触发两套插件；当前固定版本在本机列出 300 个模板，其中 230 个可使用图片。发送 `/新表情列表` 查看当前安装版的完整清单，也可以发送 `/表情搜索 关键词`、`/表情详情 关键词` 查参数和预览。例如：
...
新版单头像模板不指定目标时默认使用发送者头像；双头像模板需要再提供一个 `@群友`，缺少必要文字或图片时会回复参数提示。新版首次启动会下载约 400 MB 素材，下载完成前部分指令可能无法生成。Docker 通过独立卷保存素材、插件配置和调用统计；部署或升级后可先在宿主机执行以下命令，等出现 `Verified ... resources` 后再测试：

```powershell
docker compose run --rm --no-deps bot python scripts/sync-memes.py
docker compose up -d bot
```

本机也可运行 `.\.venv\Scripts\python.exe scripts\sync-memes.py`；只做完整性检查时加 `--verify-only`。Docker 镜像安装了 Pillow、fontconfig、FreeType 和 Noto CJK 中文字体。上述新版素材来自上游项目，其模板图片的使用权请按上游说明和实际使用场景自行确认。

群聊中还可使用头像盲盒和分页目录（无需 API Key）：

```text
/头像模板                 # 每页 18 个可直接生成的单头像模板
/头像模板 2               # 查看第 2 页
/头像盲盒                 # 随机挑一个模板，使用自己的头像
/头像盲盒 @群友           # 使用群友头像，也支持 QQ 号
```

当前固定版本有 215 个无需额外文字的单头像模板通过本机逐个生成验证。目录和盲盒只使用当前群已启用的模板，双头像或必须输入文字的模板请使用 `/新表情列表` 查询后按参数调用。头像下载设有超时和备用获取方式；下载、渲染或发送失败会尽量回复明确提示，调用统计写入失败不会阻止已生成图片发送。

新版表情配置遵循“进程环境变量 > `.env.prod` > `.env` > 默认值”；更换 `ENVIRONMENT` 时使用相应的 `.env.<环境名>`。例如在 `.env.prod` 中设置 `MEMES_COMMAND_PREFIXES=["#"]`、`MEMES_USE_SENDER_WHEN_NO_IMAGE=true`，修改后重启。`SUPERUSERS` 会保留自定义管理员并自动补入固定管理员 `2448821316`，不影响原有 `BOT_ADMIN_IDS` 权限。

新版表情统计库启动时自动检查并执行版本迁移，不使用开发模式的自动重建表逻辑。迁移前用 SQLite 在线备份保存已有数据（包含 WAL），备份位于对应数据库旁的 `backups` 目录，默认在 `data/meme-generator/localstore/data/nonebot_plugin_orm/backups`。旧版无版本号的库仅在表结构完全一致时接管；结构不匹配会明确报错并保留原数据，不会自动删表。该适配固定使用 `nonebot-plugin-orm==0.8.3`，升级此依赖前应运行迁移回归测试。迁移原则参考 [NoneBot ORM 官方说明](https://nonebot.dev/docs/best-practice/database/user)。
...
群聊发送 `今日老婆`（也支持 `随机老婆`、`抽老婆`，可加 `/` 前缀），机器人从本群
尚未绑定、未退出老婆池的群友里随机抽取一人，显示头像并 @ 对方。两人当天**双向绑定**：
双方发送 `我的老婆` 或 `查询老婆` 都会看到对方，不会被其他人重复抽中；次日北京
时间 0 点自动失效，不影响其他群。头像发送失败时自动退回文字。

双方任意一人可发送 `解绑老婆`（别名 `解绑群友老婆`）解除当天的绑定。
解绑不会重置已经用掉的每日随机抽取，但尚未抽过的一方仍可抽取。
当天随机抽取不会立刻重新配到刚解绑的同一人；双方都想恢复时仍可在解绑后使用 `强娶`。
发送 `强娶 @群友` 前，自己和目标都必须先解绑或本来就没有绑定；不会抢走别人
已有的关系，不能强娶自己或机器人。每人每天最多强娶一次，解绑也不会重置次数。
若不想被抽到或强娶，可发送 `退出老婆池`：立即解除本群今日绑定，并持续拒绝后续
配对；`加入老婆池` 可恢复参与，且不会自动恢复之前的绑定。全局黑名单中的账号也
不会被抽取或强娶。

旧版 `daily_spouses`、`daily_spouse_forces` 记录原样保留；当日旧抽取会在用户再次
发送 `今日老婆` 时尽量接入双向绑定。如果旧结果与新的一对一规则冲突，会从可用群友
中重新抽取；旧结果也可以先用 `解绑老婆` 解除，再使用 `强娶`。新绑定、解绑和退出
状态保存在现有 SQLite 数据库，重启不丢失。不需要新依赖或 API Key。
...
QQ `2448821316` 是固定机器人终极管理员，在所有群聊中保留管理员权限；
自定义管理员名单不会移除这个号码的权限。

```text
/help 或 完整指令
/help 表情                      # 也支持积分、AI、群管等分类
/指令搜索 关键词                # 搜索中央帮助菜单，最多返回 12 条
/ping
/status                         # 仅机器人终极管理员
/echo 文本                      # 仅机器人终极管理员
/reload 或 /restart             # Docker 中优雅重启；本机提示手动重启
