...
GREETING_MORNING_REPLIES='["早安！愿你今天开心 ☀️", "早上好呀，记得吃早餐！"]'
GREETING_NIGHT_REPLIES='["晚安，做个好梦 🌙", "辛苦啦，好好休息！"]'
```
...
冷却时间或每日次数限制；基础触发率为 80%，0:00～7:00 主动插话静默，模型认为没有
参与价值时不会发送。
...
为控制费用，自主回答默认遵循 DeepSeek 官方峰谷价：工作日北京时间 9:00～12:00、
14:00～18:00 为峰价，这些时段会暂时挂起 `@` 自动回答和主动插话；其他时段、周末及中国法定节假日
自动恢复，不改变群开关。`/问`、`/深度问` 等用户明确调用的指令仍可使用。若确实需要
峰价时运行，可将 `DEEPSEEK_SUSPEND_AUTOCHAT_DURING_PEAK=false`，但会增加费用。
